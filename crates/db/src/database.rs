...
pub enum RocksDbMode {
    ...
    /// On platforms without mem-env support, this uses a temp database path.
    InMemory,
}
...
fn next_in_memory_db_path() -> PathBuf {
    let id = IN_MEMORY_DB_COUNTER.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!("holo-in-memory-{id}"))
}
...
mod tests {
    ...
    use super::{
        ArtifactKey, ArtifactKind, ArtifactRecord, Database, NodeId, RocksDbDatabase, RocksDbMode,
    };
